---
name: slurm-job
description: "Submit, monitor, and collect results from SLURM jobs on HPC clusters. Use when user says \"submit job\", \"run slurm\", \"check job\", \"squeue\", or when /run-experiment or /monitor-experiment detects a SLURM environment. Handles the full lifecycle: pre-flight -> submit -> poll -> collect results."
---
...
## Submission Permission: Defer to CLAUDE.md / AGENTS.md

Before submitting any job, **read the project's `CLAUDE.md` or `AGENTS.md`** to check the current HITL policy:
- If the policy says experiments must be submitted by human operators -> show the plan and ask the user to submit manually.
- If the policy grants autonomous submission permission -> proceed with `./submit_job.sh` directly.
- If the policy is ambiguous -> default to asking the user for confirmation.

**Never hardcode the permission policy.** Always re-read the project config at submission time.
...
| Keyword | Mode |
|---------|------|
| `submit`, `run`, `deploy` | **SUBMIT** |
| `monitor`, `check`, `status`, `squeue` | **MONITOR** |
...
```
Read CLAUDE.md / AGENTS.md -> check HITL policy -> decide whether to submit directly or ask user
```
...
Read `docs/SLURM_RUNBOOK.md` (if exists) or `CLAUDE.md` to confirm:
...
### Step 10: Set Up Background Monitoring

**Immediately after successful submission**, set up a background recurring job to monitor the SLURM job automatically.

**Option A: System cron (preferred on HPC)**

```bash
# Write a monitoring script
cat > /tmp/slurm_monitor_<JOBID>.sh << 'MONITOR_EOF'
#!/bin/bash
JOB_STATE=$(squeue -j <JOBID> --noheader -o "%T" 2>/dev/null)
LOG_DIR="<project_dir>/slurm-logs"
if [ -z "$JOB_STATE" ]; then
    # Job finished
    FINAL=$(sacct -j <JOBID> --format=State --noheader | head -1 | tr -d ' ')
    echo "$(date -Iseconds) Job <JOBID> finished: $FINAL" >> "$LOG_DIR/monitor.log"
    # Self-remove from crontab
    crontab -l 2>/dev/null | grep -v "slurm_monitor_<JOBID>" | crontab -
    rm -f /tmp/slurm_monitor_<JOBID>.sh
else
    tail -5 "$LOG_DIR/<JOBID>.out" >> "$LOG_DIR/monitor.log" 2>/dev/null
fi
MONITOR_EOF
chmod +x /tmp/slurm_monitor_<JOBID>.sh

# Add to crontab (every 5 min for training, every 2 min for inference)
(crontab -l 2>/dev/null; echo "*/5 * * * * /tmp/slurm_monitor_<JOBID>.sh") | crontab -
```

**Option B: tmux background loop (when cron is unavailable)**

```bash
tmux new-session -d -s "monitor_<JOBID>" bash -c '
while true; do
    JOB_STATE=$(squeue -j <JOBID> --noheader -o "%T" 2>/dev/null)
    if [ -z "$JOB_STATE" ]; then
        sacct -j <JOBID> --format=JobID,State,ExitCode,Elapsed --noheader
        echo "Job <JOBID> finished. Monitor exiting."
        break
    fi
    tail -5 slurm-logs/<JOBID>.out 2>/dev/null
    sleep 300  # 5 minutes
done
'
```
...
**Interval guidelines:**
- Training jobs (hours): every 5 min
- Inference jobs (minutes): every 2 min
- Quick test jobs (<10 min): every 1 min

**Important:** Background monitors must self-terminate once the SLURM job completes or fails. Never leave orphan monitors running.

Report to user: "Background monitoring set up -- I'll check job <JOBID> every N minutes. Check `slurm-logs/monitor.log` or `tmux attach -t monitor_<JOBID>` for status."
...
If the job is still running and the user wants ongoing monitoring, set up a background monitor using either cron or tmux (see Step 10 in SUBMIT mode for details).

**Do NOT use tight polling loops in the main session.**
...
After results are collected, invoke `/feishu-notify` if `~/.codex/feishu.json` exists with mode != `"off"`.
...
Show the full matrix to the user. Submission follows the HITL policy.

**Important:** Add `sleep 2` between sequential SLURM submissions to avoid cache timestamp collision.
